...
var (
	...
	lastPCs []uint16;
	inst uint16; // current instruction
	memory [64*1024]uint16  // word addressing
	tim1, tim2 uint16
ips uint16
SR0, SR2 uint16
curuser, prevuser uint16
...
type page struct {
                par, pdr, addr, len, read, write, ed uint16
}
...
func switchmode(newm uint16) {
	prevuser = curuser;
	curuser = newm;
	if prevuser > 0 { USP = R[6] } else { KSP = R[6] }
	if curuser > 0 { R[6] = USP } else { R[6] = KSP }
	PS &= 0007777;
	if curuser > 0 { PS |= (1<<15)|(1<<14) }
	if prevuser > 0 { PS |= (1<<13)|(1<<12) }
}

func physread16(a uint16) uint16 {
	if a & 1 == 1 {
...
func physwrite16(a,v uint16) {
	...
	if a < 0760000 { 
		...
		PS = v;
	} else if(a == 0777546) { LKS = v } else if(a == 0777572) { SR0 = v } else if((a & 0777770) == 0777560) { conswrite16(a,v) } else if((a & 0777700) == 0777400) { rkwrite16(a,v) } else if((a & 0777600) == 0772200 || (a & 0777600) == 0777600) { mmuwrite16(a,v) } else {
		Trap(INTBUS, "write to invalid address " + ostr(a,6));
	}
}

func decode(a,w,m uint16) {
	var p page
	var user, block, disp uint16
	if(!(SR0 & 1)) {
		if(a >= 0170000) { a += 0600000 }
		return a;
	}
	if m > 0 { user = 8 } else { user = 0 }
	p = pages[(a >> 13) + user];
	if(w && !p.write) {
		SR0 = (1<<13) | 1;
		SR0 |= (a >> 12) & ^1;
	...
	block = (a >> 6) & 0177;
	disp = a & 077;
	if p.ed > 0 && block < p.len || p.ed == 0 && block > p.len {
...
func mmuread16(a uint16) uint16 {
	var i uint16
	i = (a & 017)>>1;
...
func write8(a, v uint16) uint16 {
	return physwrite8(decode(a, true, curuser),v);
}

func write16(a, v uint16 )  uint16 {
...
func printstate() {
	writedebug(
		...
		"R6 " + ostr(R[6],6) + " " + 
		"R7 " + ostr(R[7],6) + "\n[");
	if(prevuser != 0) { writedebug("u")} else {writedebug("k"); }
	if(curuser != 0) { writedebug("U") } else { writedebug("K") }
	if(PS & FLAGN != 0) { writedebug("N") } else { writedebug(" ")}
	if(PS & FLAGZ != 0) { writedebug("Z") } else { writedebug(" ")}
	if(PS & FLAGV != 0) { writedebug("V") } else { writedebug(" ")}
	if(PS & FLAGC != 0) { writedebug("C") } else { writedebug(" ")}
...
func Trap(num uint16, msg string) { 
	panic(struct {num uint16; msg string} { num, msg })
}
...
func handleinterrupt(vec uint16) {
	defer func() {
		...
		case struct{num uint16; msg string}:
...
func step() {
	var val, val1, val2, ia, da, sa, d, s, l, r, o, max, maxp, msb uint16
	ips++;
	if(waiting) { return }
	curPC = R[7];
	...
	case 0005400: // NEG
		...
		PS &= 0xFFF0;
		if(val & msb) { PS |= FLAGN }
		if(val == 0) { PS |= FLAGZ } else { PS |= FLAGC }
	...
	case 0006600: // MTPI
		...
		val = pop();
		if(da == -7) {
			if(curuser == prevuser) { R[6] = val } else if prevuser != 0 { USP = val; }  else {KSP = val }
		} else if(da < 0) {
